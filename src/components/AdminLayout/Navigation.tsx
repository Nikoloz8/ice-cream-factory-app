...
export default function Navigation() {
    ...
    return (
        <nav className="w-[230px]">
            ...
            {navButtons.map((e, i) => {
                return <button key={i} onClick={() => navigate(`/administrator/${e}`)} className={`w-[100%] text-[1.4rem] font-[500] leading-[100%] text-[rgba(255,255,255,0.8)] bg-[rgba(4,174,210,1)] p-[8px_0px_8px_21px] outline-none cursor-pointer text-left border-b-[1px] border-solid border-[rgba(248,231,243,1)] ${useMatch(`/administrator/${e}`) ? "text-[rgba(255,255,255,1)]! font-[700]!" : undefined} ${i === 0 ? "rounded-[8px_8px_0_0]" : undefined} ${i + 1 === navButtons.length ? "rounded-[0_0_8px_8px]" : undefined}`}>
                    {e}
                </button>
            })}
        </nav>)
