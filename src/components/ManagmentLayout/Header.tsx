    ...
    return (
        <header className="flex bg-[rgba(0,58,70,1)] justify-center items-center p-[16px_64px_16px_64px]">
            <div className="flex w-[1110px] items-center justify-between">
                ...
                {useMatch(`/login&register/register/${getRole()}`) || useMatch(`/login&register/login/${getRole()}`) ? <button onClick={() => navigate("/")} className={`p-[10px_19px] border-[0.55px] border-solid border-[rgba(118,229,252,1)] cursor-pointer rounded-[6.27px] text-[1.1rem] font-[400] leading-[100%] text-[rgba(118,229,252,1)]`}>უკან</button> : <button onClick={() => {
                    navigate("/")
